...
export async function assert(argumentName: string, actualValue: any, assertionType: string, expectedValue: any) {
  try {
    if (assertionType === 'toMatchRegex') {
      const regex = new RegExp(expectedValue);
      if (!regex.test(actualValue)) {
        throw new Error(`Value '${actualValue}' does not match the expected regex '${expectedValue}'`);
      }
    } else {
      await expect(actualValue)[assertionType](expectedValue);
    }
    console.log(color.assert(`Assertion Passed: expected [${argumentName}: ${actualValue}] ${assertionType} [${expectedValue}]`));
  } catch (error) {
    console.log(color.error(`Assertion Failed: expected [${argumentName}: ${actualValue}] ${assertionType} [${expectedValue}]`));
    throw error;
  }
};
